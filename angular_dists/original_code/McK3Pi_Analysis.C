#define McK3Pi_Analysis_cxx
#include "McK3Pi_Analysis.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <TSystem.h>
...
void McK3Pi_Analysis::Loop()
{
//   In a ROOT session, you can do:
//      Root > .L McK3Pi_Analysis.C
//      Root > McK3Pi_Analysis t
...
TH1 * d0MassHist               = new TH1D("d0MassHist","   ",120,1800.,1920.);
TH1 * d0MassHistSelected       = new TH1D("selectedD0MassHist","   ",120,1800.,1920.);
TH1 * d0barMassHist               = new TH1D("d0barMassHist","   ",120,1800.,1920.);
TH1 * d0barMassHistSelected       = new TH1D("selectedD0barMassHist","   ",120,1800.,1920.);
...
TH2D * mABmCD                   = new TH2D("mABmCD","passing cutsB",50,550,1300.,50,200.,1300.);
TH2D * mABmCDForLowKPiPi        = new TH2D("mAB_v_mCD for Low KPiPi","passing cutsB",50,550,1300.,50,200.,1300.);
TH2D * mADmCB                   = new TH2D("mADmCB","passing cutsB",50,550,1550,50,200.,1300.);
...
TH2D * mABmAD                   = new TH2D("mABmAD","passing cutsB",50,550.,1300.,55,550.,1650.);
TH2D * mABmADForKappaRho         = new TH2D("mAB_v_mA, for Kappa RhoD","passing cutsB",50,550.,1300.,55,550.,1650.);
TH2D * mABmADForKstarRho        = new TH2D("mAB_v_mAD, K*Rho","passing cutsB",50,550.,1300.,55,550.,1650.);
TH2D * mABmADForKstarF0         = new TH2D("mAB_v_mAD, K* f0","passing cutsB",50,550.,1300.,55,550.,1650.);
TH2D * mABmADzoomed             = new TH2D("mABmADzoomed","passing cutsB",25,600,850,25,600.,850.);
...
TH1D * piMinusEnergyHist     = new TH1D("piMinusEnergyHist","passing cutsB",80,100.,900.);
...
TH1D * cornerKPiPiMassSqHist       = new TH1D("KPiPiMassSqCorner","for low KPiPi mass",100,0.4,3.4);
TH1D * KPiPiMassSqHist             = new TH1D("KPiPiMassSqHist","for cutsB",100,0.4,3.4);
TH1D * cornerKminusHelictyCosineHist = new TH1D("KminusHelictyCosineHist corner","in the corner",100,-1.,1.);
TH1D * KPiPiMassHist               = new TH1D("KPiPiMassHist"," ",100,750.,1750.);
...
//    and calculate the sine of the angle between the (K-.pi+) plane and the (pi-,pi+) plane
  ...
  bool cutsB = true;
      if (cutsB) {
        kminus_4vec.SetPxPyPzE(Kplus_Px, Kplus_Py, Kplus_Pz, Kplus_E);
        piminus_4vec.SetPxPyPzE(Piplus_Px, Piplus_Py, Piplus_Pz, Piplus_E);
        piplus1_4vec.SetPxPyPzE(Piminus1_Px, Piminus1_Py, Piminus1_Pz, Piminus1_E);
        piplus2_4vec.SetPxPyPzE(Piminus2_Px, Piminus2_Py, Piminus2_Pz, Piminus2_E);
...
//  create the D0 daughter four-vectors, first in the lab, and then boosted into the D0 CM
      ...
      KPiPiMassHist->Fill(pABD_4vec.M());
...
//  rather than phi above or below pi.
   ...
   Double_t maximum = quad1PhiHistD02KstarRho->GetMaximum();
    cout << "max from quad1PhiHistD02KstarRho->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad2PhiHistD02KstarRho->GetMaximum();
    cout << "max from quad2PhiHistD02KstarRho->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad3PhiHistD02KstarRho->GetMaximum();
    cout << "max from quad3PhiHistD02KstarRho->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad4PhiHistD02KstarRho->GetMaximum();
    cout << "max from quad4PhiHistD02KstarRho->GetMaximum() = " << maximum << endl;
...
//  rather than phi above or below pi.
   ...
   maximum = quad1PhiHistD02KstarF0->GetMaximum();
    cout << "max from quad1PhiHistD02KstarF0->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad2PhiHistD02KstarF0->GetMaximum();
    cout << "max from quad2PhiHistD02KstarF0->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad3PhiHistD02KstarF0->GetMaximum();
    cout << "max from quad3PhiHistD02KstarF0->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad4PhiHistD02KstarF0->GetMaximum();
    cout << "max from quad4PhiHistD02KstarF0->GetMaximum() = " << maximum << endl;
...
//  rather than phi above or below pi.
   ...
   maximum = quad1PhiHistD02KappaRho->GetMaximum();
    cout << "max from quad1PhiHistD02KappaRho->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad2PhiHistD02KappaRho->GetMaximum();
    cout << "max from quad2PhiHistD02KappaRho->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad3PhiHistD02KappaRho->GetMaximum();
    cout << "max from quad3PhiHistD02KappaRho->GetMaximum() = " << maximum << endl;
...
//  do asymmetry calculations before re-scaling the distributions
   ...
   maximum = quad4PhiHistD02KappaRho->GetMaximum();
    cout << "max from quad4PhiHistD02KappaRho->GetMaximum() = " << maximum << endl;
